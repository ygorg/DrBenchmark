...
_SPECIALITIES = ['immunitaire', 'endocriniennes', 'blessures', 'chimiques', 'etatsosy', 'nutritionnelles', 'infections', 'virales', 'parasitaires', 'tumeur',
                 'osteomusculaires', 'stomatognathique', 'digestif', 'respiratoire', 'ORL', 'nerveux', 'oeil', 'homme', 'femme', 'cardiovasculaires', 'hemopathies', 'genetique', 'peau']

_LABELS_BASE = ['anatomie', 'date', 'dose', 'duree', 'examen', 'frequence',
                'mode', 'moment', 'pathologie', 'sosy', 'substance', 'traitement', 'valeur']

_URL = "https://huggingface.co/datasets/DrBenchmark/DEFT2021/resolve/main/data.zip"
...
class DEFT2021(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(name="cls", version="1.0.0",
                               description="DEFT 2021 corpora - Classification task"),
        datasets.BuilderConfig(name="ner", version="1.0.0",
                               description="DEFT 2021 corpora - Named-entity recognition task"),
        datasets.BuilderConfig(name="ner_long", version="1.0.0",
                               description="DEFT 2021 corpora - Named-entity recognition task"),
    ]

    def _info(self):

        if "cls" in self.config.name:
    ...
    def split_sentences(self, json_o):
        ...
        for i in json_o:
            ...
            ind_punc = [index for index, value in enumerate(
                i['tokens']) if value == '.'] + [len(i['tokens'])]
    ...
    def _generate_examples(self, data_dir, split):
        ...
        if "cls" in self.config.name:
            ...
            with open(os.path.join(data_dir, 'distribution-corpus.txt')) as f_dist:
                ...
                random.seed(4)
                train = [raw.split('\t')[0] for raw in distribution if len(
                    raw.split('\t')) == 4 and raw.split('\t')[3] == 'train 2021']
                random.shuffle(train)
                random.shuffle(train)
                random.shuffle(train)
                train, validation = np.split(train, [int(len(train) * 0.7096)])

                test = [raw.split('\t')[0] for raw in distribution if len(
                    raw.split('\t')) == 4 and raw.split('\t')[3] == 'test 2021']
            ...
            with open(os.path.join(data_dir, 'distribution-corpus.txt')) as f_dist:
                ...
                for guid, txt_file in enumerate(sorted(ann_path.glob("*.txt"))):
                    ...
                    hf_split = hf_format
                    if "long" not in self.config.name:
