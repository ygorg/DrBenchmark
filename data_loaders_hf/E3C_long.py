...
class E3C_long(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS += [
        datasets.BuilderConfig(name=f"{lang}_clinical_long", version="1.0.0", description=f"The {lang} subset of the E3C corpus") for lang in _LANGUAGES
    ]

    BUILDER_CONFIGS += [
        datasets.BuilderConfig(name=f"{lang}_temporal_long", version="1.0.0", description=f"The {lang} subset of the E3C corpus") for lang in _LANGUAGES
    ]
    ...
    def _info(self):
        ...
        if self.config.name.find("clinical") != -1:
            ...
            names = ["O", "B-EVENT", "B-ACTOR", "B-BODYPART", "B-TIMEX3",
                     "B-RML", "I-EVENT", "I-ACTOR", "I-BODYPART", "I-TIMEX3", "I-RML"]
    ...
    def _split_generators(self, dl_manager):
        ...
        lang = self.config.name.split('_')[0]
        ...
        if self.config.name.find("clinical") != -1:
            ...
            return [
                datasets.SplitGenerator(
                    ...
                    gen_kwargs={
                        "filepath": os.path.join(data_dir, "E3C-Corpus-2.0.0/data_annotation", lang, "layer2"),
                        "split": "train",
                    },
                ),
                datasets.SplitGenerator(
                ...
                datasets.SplitGenerator(
                    ...
                    gen_kwargs={
                        "filepath": os.path.join(data_dir, "E3C-Corpus-2.0.0/data_annotation", lang, "layer1"),
                        "split": "test",
                    },
                ),
            ]
    ...
    def _generate_examples(self, filepath, split):
        ...
        for content in parsed_content:
            ...
            if "long" in self.config.name:
                # Creating one sentence with the whole ument.
                content['SENTENCE'] = [[
                    content['SENTENCE'][0][0],
                    content['SENTENCE'][-1][1],
                    content['TEXT']
                ]]
