...
class QUAERO_long(datasets.GeneratorBasedBuilder):
    """QUAERO dataset."""
    ...
    BUILDER_CONFIGS = [
        ...
        datasets.BuilderConfig(name="emea_long", version=VERSION, description="The EMEA QUAERO corpora"),
        datasets.BuilderConfig(name="medline", version=VERSION,
                               description="The MEDLINE QUAERO corpora"),
    ]
    ...
    def _info(self):
                ...
                features=datasets.Features(
                    {
                        ...
                        "ner_tags": datasets.Sequence(
                            datasets.features.ClassLabel(
                                names=['O', 'B-LIVB', 'I-LIVB', 'B-PROC', 'I-PROC', 'B-ANAT', 'I-ANAT', 'B-DEVI', 'I-DEVI', 'B-CHEM', 'I-CHEM',
                                       'B-GEOG', 'I-GEOG', 'B-PHYS', 'I-PHYS', 'B-PHEN', 'I-PHEN', 'B-DISO', 'I-DISO', 'B-OBJC', 'I-OBJC'],
                            )
                        ),
                    }
                ),
        ...
        elif 'medline' in self.config.name:
            return datasets.DatasetInfo(
                description=_DESCRIPTION,
                features=datasets.Features(
                    {
    ...
    def _generate_examples(self, split):
        ...
        ds = load_dataset(
            "bigbio/quaero", f"quaero_{'emea' if 'emea' in self.config.name else 'medline'}_source",
        ...
        if self.config.name != "emea_long":
            ds = self.split_sentences(ds)
